...
class UnpackUtils
{
    ...
    static long unpack_samples(WavpackContext wpc, int [] buffer, long sample_count, int bufferStartPos)
    {
        ...
        long i;
        int crc = (int) wps.crc;
        final int byte_count = wpc.config.bytes_per_sample;
        ...
        if ((flags & (Defines.MONO_FLAG | Defines.FALSE_STEREO)) > 0)
        {
            ...
            if(byte_count>2)
            {
                for (tcount = wps.num_terms - 1; tcount >= 0; tcount--)
                {
                    ...
                    decorr_mono_pass_24bit(dpp, buffer, sample_count, bufferStartPos);
                    dpp_index++;
                }    
            }
            else
            ...
            int bf_abs;
            final int crclimit = (int)(sample_count + bufferStartPos);
        ...
        else
        {
            ...
            if (sample_count < 16)
            {
                ...
                if(byte_count>2)
                {
                    ...
                    {
                        dpp = wps.decorr_passes[dpp_index];
                        decorr_stereo_pass_24bit(dpp, buffer, sample_count, bufferStartPos);
            ...
            else
            {
                ...
                if(byte_count>2)
                {
                    ...
                    {
                        dpp = wps.decorr_passes[dpp_index];
                        decorr_stereo_pass_24bit(dpp, buffer, 8, bufferStartPos);

                        decorr_stereo_pass_cont_24bit(dpp, buffer, sample_count - 8, bufferStartPos + 16);
                        wps.decorr_passes[dpp_index] = dpp;
    ...
    static void decorr_stereo_pass(decorr_pass dpp, int [] buffer, long sample_count, int buf_idx)
    {
        ...
        int m, k;
        int bptr_counter = 0;
        final int end_index = (int)(buf_idx + sample_count * 2);
    ...
    static void decorr_stereo_pass_24bit(decorr_pass dpp, int [] buffer, long sample_count, int buf_idx)
    {
        int delta = dpp.delta;
        int weight_A = dpp.weight_A;
        int weight_B = dpp.weight_B;
    ...
    static void decorr_stereo_pass_cont_24bit(decorr_pass dpp, int [] buffer, long sample_count, int buf_idx)
    {
        int delta = dpp.delta, weight_A = dpp.weight_A, weight_B = dpp.weight_B;
        int tptr;
        int sam_A, sam_B;
    ...
    static void decorr_mono_pass(decorr_pass dpp, int [] buffer, long sample_count, int buf_idx)
    {
        ...
        final int end_index = (int)(buf_idx + sample_count);

        switch (dpp.term)
        {
            case 17:
                for (bptr_counter = buf_idx; bptr_counter < end_index; bptr_counter++)
                {
                    sam_A = 2 * dpp.samples_A[0] - dpp.samples_A[1];
                    dpp.samples_A[1] = dpp.samples_A[0];
                    dpp.samples_A[0] = ((weight_A * sam_A + 512) >> 10) + buffer[bptr_counter];
            ...
            default:
                for (m = 0,
                k = dpp.term & (Defines.MAX_TERM - 1),
                bptr_counter = buf_idx; bptr_counter < end_index; bptr_counter++)
                {
                    sam_A = dpp.samples_A[m];
                    dpp.samples_A[k] = ((weight_A * sam_A + 512) >> 10) + buffer[bptr_counter];
    ...
    static void decorr_mono_pass_24bit(decorr_pass dpp, int [] buffer, long sample_count, int buf_idx)
    {
        int delta = dpp.delta, weight_A = dpp.weight_A;
        int sam_A;
        int m, k;
    ...
    static void fixup_samples(WavpackStream wps, int [] buffer, long sample_count, int bufferStartPos)
    {
        long flags = wps.wphdr.flags;
        ...
        if ((flags & Defines.FLOAT_DATA) > 0)
        {
            ...
            FloatUtils.float_values(wps, buffer, sc, bufferStartPos);
        }
